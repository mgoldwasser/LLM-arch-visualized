...
export function render({ id, num, title }) {
  ...
  return chapter(id,
    ...
    mathAside('a neuron is a hyperplane with a steepness dial', `
      ...
      <p>and for any point the signed perpendicular distance to H is u(<strong>x</strong>) / ‖<strong>w</strong>‖. Two consequences are worth holding onto. First, H itself depends only on the <em>direction</em> of <strong>w</strong> and on the ratio b/‖<strong>w</strong>‖: the plane sits at distance −b/‖<strong>w</strong>‖ from the origin along <strong>w</strong>. Second, scaling <strong>w</strong> up by a factor k does not move H at all — it multiplies every distance by k, which makes σ switch from one extreme to the other over a k-times narrower strip. Growing the weights of a sigmoid neuron sharpens it toward a hard step; shrinking them softens it toward a straight ramp. The bias moves the boundary, the weight magnitude sets how abrupt it is, and only the weight direction chooses what the question is about.</p>`),

    /* ---- 2 · a layer is the matrix product, plus σ -------------------------- */
    prose(
      `<strong>A layer is not a new idea.</strong> Put <em>k</em> neurons side by side, all reading the same input <strong>x</strong>. Each contributes one dot product, so the layer produces <em>k</em> numbers — and <em>k</em> dot products against <em>k</em> weight vectors is precisely the matrix product from the previous chapter. Stack the weight vectors as the columns of a matrix <em>W</em>, collect the biases into a vector <strong>b</strong>, and the layer is <em>h</em>&nbsp;=&nbsp;σ(<em>xW</em>&nbsp;+&nbsp;<strong>b</strong>), with σ applied to each coordinate independently. The linear algebra is identical to what you already have; the layer adds exactly one thing, an elementwise function on the way out. Every &ldquo;neural network&rdquo; diagram of circles and arrows you have ever seen is this expression, drawn the long way.`,
    ...
    term('decision boundary', 'n.', 'the set of inputs on which a classifier is exactly undecided — the level set where its score is zero; for an affine map this set is always flat'),
...
z = hW₂ + b₂
  ...
  = xM + c</div>
    ...
    prose(
      `With σ off, no arrangement of the six sliders produces a bent boundary. You can move the straight line, tilt it, push it out of the frame — but every setting gives the same family of parallel level sets, because every setting computes the same kind of function. The nonlinearity is not a performance tweak or a training trick. It is the difference between a model class that contains the answer and one that does not.`),
