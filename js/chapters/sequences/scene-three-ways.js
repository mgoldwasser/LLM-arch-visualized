...
export function threeWaysScene() {
  return createScene({
    ...
    figure: sceneFigure,
    steps: [
      { n: 'STEP 1 / 4 — COUNT', html: `<p>The n-gram model keeps a fixed window — ${NGRAM} tokens here — and estimates the next-token probability by counting how often that window was followed by each token in a corpus. Slide the window along sentence A and every count is healthy.</p><p>Sentence B is the same English with one ordinary noun swapped in. Every window touching it counts zero — and zero on top is not a small probability, it is none. Smoothing patches the arithmetic but cannot supply the knowledge: nothing the table learned about &ldquo;the cat sat&rdquo; connects to &ldquo;the wombat sat&rdquo;.</p>` },
      ...
      { n: 'STEP 3 / 4 — SHRINK', html: `<p>Now run training backwards. To reach an early hidden state, the gradient must pass back through every step in between, and each of those steps multiplies it by the same matrix. Multiplying by a factor below 1, over and over, shrinks the signal geometrically; a factor above 1 blows it up.</p><p>At a per-step norm of ${GAMMA.toFixed(2)}, the correction reaching ${T} steps back is already down to ${(GAMMA ** T).toFixed(2)} of full strength; a hundred steps back it is ${sci(GAMMA ** 100)}, far beneath the floating-point noise floor. The long-range dependency is exactly the one that cannot be learned.</p>` },
