...
export function render({ id, num, title }) {
  ...
  return chapter(id,
    ...
    prose(
      ...
      `What neither move does is <em>compare</em> two vectors and return a verdict. Yet comparison is what the model spends its life doing, in different costumes. Does this token's vector match the pattern that some part of the model is watching for — the question that decides which earlier words this one pulls in (${chRef('attention')})? Does it match the kind of material one of the model's specialist sub-networks was trained to handle (${chRef('moe')})? Which of ${(K3.vocab / 1000).toFixed(0)},000 possible next words does it point toward? Every one of those is the same question — how much do two vectors agree? — and every one is answered by the same arithmetic: multiply matching coordinates, add up the results, report one number.`),

    term('dot product', 'n.', 'multiply matching coordinates and add up the results — a·b = Σᵢ aᵢbᵢ. Takes two vectors of the same dimension, returns one number (a <em>scalar</em>), which measures how much the two agree in direction'),
    ...
    term('norm', 'n.', 'the length of the arrow — |a| = √(Σᵢ aᵢ²). A <strong>unit vector</strong> has |a| = 1; â = a/|a| is a’s direction with its magnitude discarded'),
    ...
    prose(
      ...
      `One last question, easy to skip and worth asking: why is the machine built out of <em>this</em> operation rather than some more expressive one? Partly because the dot product is the natural measure of agreement in a vector space. Mostly because of silicon. A matrix multiply is a very large number of dot products that all reuse the same operands, so each value fetched from memory feeds many multiply-accumulates — the one shape of computation that modern accelerators execute at extraordinary rates — on the order of 10¹⁵ multiply-adds per second, a thousand trillion, on a single flagship chip, provided each number is stored in only a handful of bits rather than the usual thirty-two (<em>low precision</em>). Operations without that structure are bandwidth-starved by comparison. That constraint shaped the machine. A <strong>transformer</strong> — the design named in ${chRef('objective')} and assembled in ${chRef('residual')} — is a stack of near-identical layers, each one letting the tokens read each other and then reworking each token on its own, and it is built almost entirely out of matrix multiplies. It is not the architecture that best expresses language in the abstract. It is the architecture that expresses language <em>well enough</em> using the operation hardware can do at absurd throughput. That trade — the subject of ${chRef('attention')}, where a dot product becomes the entire mechanism for deciding what a token pays attention to — is the reason the field looks the way it does.`),

    takeaway(
      `Everything ahead is dot products in costume. How much one token should draw on another — its <em>attention score</em> — is a dot product between two versions of them, one asking and one answering; those two are called its <em>query</em> and its <em>key</em>, and ${chRef('attention')} builds them. Which specialist sub-network a token is sent to is settled by a <em>router score</em>: a dot product between that token and the one direction each sub-network learned to stand for (${chRef('moe')}). How strongly the model favours a particular next word is a dot product between the token's final vector and one row of the output matrix — one row per word in the vocabulary; those scores are the <em>logits</em> of ${chRef('probability')}. A layer is a few enormous piles of them, executed at once as matrix multiplies. Direction carries the meaning, sign carries the verdict, and a weight matrix is nothing more than a warehouse of learned directions to be compared against.`));
}
