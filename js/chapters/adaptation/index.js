...
export function render({ id, num, title }) {
  return chapter(id,
    ...
    prose(
      `<strong>TinyLoRA</strong> (2026 — FAIR, Cornell, CMU; “Learning to Reason in 13 Parameters”) asks the reductio question: is even rank one necessary? Standard LoRA can't shrink below the model's width — B alone is d×1. TinyLoRA escapes that floor in three moves. First, stop letting the update point in any direction it likes. Every weight matrix has a handful of directions along which it does most of its work, and a standard piece of linear algebra — the <strong>SVD</strong> — finds them. Compute them once from the frozen W, never touch them again, and require the update to live inside that small space: now it takes only r×r numbers, sandwiched between two fixed factors, rather than a whole matrix. Second, do not train even those r×r numbers directly. Generate them from a tiny trainable vector v, pushed through a <strong>fixed random projection</strong> — a scrambling drawn at random once, at the start, and never trained. Third, <strong>weight tying</strong>: share one v across modules and layers (“tiled” across depth works best). The trainable count becomes whatever length you choose for v — down to one.`),
    ...
    prose(
      `Honest caveats: the result lives where rewards are verifiable (math, code) — subjective domains may not compress this way; Qwen models respond to tiny budgets ~10× more readily than Llama for reasons nobody has pinned down; and the paper's own curiosities (below about a kilobyte of update, keeping each number at full 32-bit precision beats spending the same total bits on twice as many half-precision numbers) mark this as an early map of a strange regime, not a finished theory. But the trendline is unambiguous — and it scales the right way: the bigger the base model, the fewer parameters adaptation seems to need. For a ${si(K3.totalParams)}-parameter ${K3.name}, “fine-tuning” may eventually mean a payload smaller than this paragraph.`),
