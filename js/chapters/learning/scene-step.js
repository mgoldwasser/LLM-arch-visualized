...
export function sceneStep() {
  return createScene({
    ...
    steps: [
      ...
      { n: 'STEP 3 / 5 — BACKWARD', html: `<p>Now walk backwards. Each node receives the gradient of L with respect to its own output and multiplies by its local derivative to produce the gradient with respect to its input. Nothing else happens — no search, no trial and error, five lines of arithmetic.</p><p>Notice ∂L/∂a = W⁽²⁾ᵀ(∂L/∂z₂): the transposed weight matrix carries the error signal one layer back. And ∂L/∂W⁽¹⁾ = (∂L/∂z⁽¹⁾)xᵀ uses the cached x. That is why the forward pass had to save it.</p>` },
