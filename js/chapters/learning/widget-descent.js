...
export function descentWidget() {
  ...
  return el('div', {},
    ...
    node, stats,
    el('div', { class: 'w-note' },
      `The two axes have curvatures 1 and ${KAPPA}, so the largest stable learning rate is 2/${KAPPA} = ${DIVERGE.toFixed(2)} — set by the steepest direction, while the shallow one then converges ${KAPPA} times slower. That gap is the whole difficulty of choosing η, and real loss surfaces have curvature ratios in the millions.`));
}
